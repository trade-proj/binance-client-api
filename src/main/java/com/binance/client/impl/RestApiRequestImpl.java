...
class RestApiRequestImpl {
  ...
  List<Candlestick> getCandlestickSpot(String symbol, CandlestickInterval interval, Long startTime,
    Long endTime, Integer limit) {
    ...
    parameters.put("limit", limit == null ? 10 : limit);

    String responseString = client.createMarket().klines(parameters);
    JsonWrapper response = JsonWrapper.parseFromString(responseString);
    List<Candlestick> result = new LinkedList<>();
    JsonWrapperArray dataArray = response.getJsonArray("data");
    dataArray.forEachAsArray((item) -> {
      Candlestick element = new Candlestick();
      element.setOpenTime(item.getLongAt(0));
      element.setOpen(item.getBigDecimalAt(1));
